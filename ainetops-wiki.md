...
| AI agent lifecycle — benchmarking, grounding | Gap | Benchmarking: coordinate with BMWG; grounding: no current work |
...
When evaluating whether to adopt a submission as a work item as part of a proposal for the Proposed New WG, I encourage chairs to apply the following criteria. These are also the criteria chairs and I will use together when assessing whether a proposal is ready to move forward:
...
- **Existing WG scope confirmation.** OPSAWG's, NMOP's, and NETCONF WG's current charters do not cover AI agent management for network operations as their own scope, and expanding any of them to absorb this body of work is not the proposed path. This is not an open per-item assessment — the side meeting should confirm rough consensus on this point and log any dissent as a known objection.

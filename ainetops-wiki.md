...
*v1.4 · Last updated: 11 September 2026*

> **AINETOPS BoF Proposal — IETF 127**  
>  A draft BoF proposal for a WG-forming "AI-enabled Network Operations" (AINETOPS) BoF is posted for discussion on the mailing list (draft, not yet submitted). It proposes two Standards Track deliverables — YANG models for **agent operational state and lifecycle**, and for **agent task supervision and intervention** — plus supporting Operational Framework/Requirements and Operational Guidance documents. Scope is the management interface between a supervising system and network-management agents, not the agent's internal model, prompts, training, or reasoning.  
>  Key dates: initial BoF request due **18 September 2026**; revised requests due **2 October 2026**; AD approval due **9 October 2026**.  
>  Full proposal: <https://github.com/dan-voyer/IETF/blob/main/AINETOPS-proposal/AINETOPS-BoF-Proposal-IETF127.md>

> **Webex Side Meeting 08/10/2026**  
>  [Webex Recording](%5Bhttps://futurewei.my.webex.com/futurewei.my/ldr.php?RCID=b1c00991226b3495e57ecef0041e4aca) (password: cGBPEfh5 )  
...
> **Status update (as of September 2026): neither has formed a working group yet.** Both DAWN and AGENTPROTO passed their WG-forming BoFs at IETF 126 and are now in IESG chartering review — proposed WGs, not active ones. DAWN's draft charter is on an IESG ballot with 2 BLOCK positions still outstanding (enough support exists to pass once those are resolved, but no telechat date is set); AD is Éric Vyncke, chairs Adrian Farrel and Wes Hardaker. AGENTPROTO is similarly in "Start Chartering/Rechartering (IESG/IAB Review)"; AD is Charles Eckel (ART area), chairs Leslie Daigle and Orie Steele. Treat references below to "coordinate with DAWN" or "AGENTPROTO forms a working group" as pointing at these still-pending charters, not established venues.
...
[`draft-carpenter-anima-grasp-rendezvous`](https://becarpenter.github.io/grasp-rendezvous/draft-carpenter-anima-grasp-rendezvous.html) provides a concrete example of protocol-anchored A2A work in this space: two GRASP-based rendezvous procedures built directly on [RFC 8990](https://datatracker.ietf.org/doc/rfc8990/), with a working prototype. It addresses discovery/bootstrapping, not capability advertisement within an established session — see the cross-reference under Agent Discovery for how this relates to DAWN. The draft explicitly scopes out authentication and authorization between rendezvous participants — a gap that is significant for AI agent deployments, where the authorization model governing what one agent can ask another to do carries governance implications beyond those of traditional autonomous agents. Work building on this draft should address that gap, either in scope or by identifying a companion work item.

> **Open question — see AGENTPROTO BoF.** The AGENTPROTO BoF (WG-forming BoF held IETF 126; charter still in IESG review, not yet chartered) proposes A2A communication protocols built on MoQ (Media over QUIC) rather than NETCONF/RESTCONF. If AGENTPROTO's charter clears IESG review and the working group forms, it may define the A2A protocol standard that OPS area management work would reference rather than define independently. Whether network management A2A interfaces require separate treatment in OPSAWG, NMOP, or the Proposed New WG — or whether they are a use case AGENTPROTO would cover — is an open question the side meeting should address.
...
The community has identified a set of AI agent lifecycle topics not yet well-addressed by current submissions and representing a gap in the existing work:
  ...
  - **Model upgrade and lifecycle management.** How an AI agent's underlying model is updated, rolled back, or replaced without disrupting ongoing network operations, and how the impact of a model change is assessed. [RFC 9222](https://datatracker.ietf.org/doc/rfc9222/) Section 7 addresses upgrade and lifecycle management for autonomous agents and is directly relevant existing work in this area. The residual question is whether LLM-specific scenarios — model version rollback distinct from software upgrades, impact assessment of a model change on network behavior, model grounding preservation across upgrades — introduce considerations beyond [RFC 9222](https://datatracker.ietf.org/doc/rfc9222/)'s current scope.
...
AI agent lifecycle — grounding | Problem statement exists | See [draft-fu-nmop-tokenops-probelem-statement](https://datatracker.ietf.org/doc/draft-fu-nmop-tokenops-probelem-statement) (China Telecom); normative work not yet started  
AI agent lifecycle — upgrade and lifecycle management | Partial — see [RFC 9222](https://datatracker.ietf.org/doc/rfc9222/) §7 | [RFC 9222](https://datatracker.ietf.org/doc/rfc9222/) §7 covers autonomous agent upgrade and lifecycle; residual gap: LLM-specific scenarios (model version rollback, model change impact assessment)  
